...
static void destroySDL(SDL_Window *window, SDL_Renderer *renderer, SDL_Texture *tab_texture[NB_LIGNE][NB_COLONNE]){
    if(NULL != renderer)
...
static unsigned playHumainGraphique(Puissance4 *game) {
  int coup;
  int grid_cell_width = (WIDTH / 7)-30;
  int width_plateau = WIDTH-(30*7);
  assert(game->courant);
  //(game->courant->type == J1) ? (printf("Joueur 1 : ")) : (printf("Joueur 2 : "));
  SDL_Event event;
...
static void updateGraphique(void *data, Puissance4 *game) {
  ...
  int grid_cell_width = (WIDTH / 7)-30;
  int grid_cell_height = HEIGHT / 6;
  SDL_Color blanc = {255, 255, 255, 255};
  SDL_Color jaune = {227, 195, 16, 255};
  SDL_Color rouge = {222, 61, 40, 255};
  SDL_Rect rect = {(game->colonne * grid_cell_width)+5, (game->ligne * grid_cell_height)+5, grid_cell_width-10, grid_cell_height-10};
  ...
  switch (game->plateau[game->ligne][game->colonne]) {
    case J1:
      ...
      SDL_RenderCopy(d->renderer, d->tab_texture[game->ligne][game->colonne], NULL, &rect);
      break;
    case J2:
